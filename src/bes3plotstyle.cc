...
void bes3plotstyle::MakeLegend(TH1* datahist,   // Histogram with data
    ...
    if (datahist && dataname) leg->AddEntry(datahist, dataname, "LEP");
    if (mc1hist && mc1name) leg->AddEntry(mc1hist, mc1name, "L");
    if (mc2hist && mc2name) leg->AddEntry(mc2hist, mc2name, "LF");
...
void bes3plotstyle::MakeLegend(TGraph* datahist,  // Graph with data
    ...
    if (datahist && dataname) leg->AddEntry(datahist, dataname, "LEP");
    if (mc1hist && mc1name) leg->AddEntry(mc1hist, mc1name, "L");
    if (mc2hist && mc2name) leg->AddEntry(mc2hist, mc2name, "L");
    if (mc3hist && mc3name) leg->AddEntry(mc3hist, mc3name, "L");
...
void bes3plotstyle::PlotDataMC(char* filename,  // Name for the output files,
    ...
    if (mc1hist) bes3plotstyle::FormatMC1(mc1hist);
    if (mc2hist) bes3plotstyle::FormatMC2(mc2hist);

    datahist->Draw("axis");
    if (mc2hist) mc2hist->Draw("same");
    if (mc1hist) mc1hist->Draw("same");
    datahist->Draw("Esame");
    datahist->Draw("axissame");
    if (prelim) {
        WriteBes3();
        if (prelim == 1) WritePreliminary();
    }
    MakeLegend(datahist, dataname, mc1hist, mc1name, mc2hist, mc2name);
...
void bes3plotstyle::PlotDataFit(char* filename,  // Name for the output files,
                                ...
                                char* dataname,        // Description of data
                                char** functionnames,  // Names of associated
                                                       // functions
                                Int_t prelim,  // Use 1 for Preliminary plot
                                // 2 for a publication plot
                                // and 0 for a meeting plot with
...
/*
   void bes3plotstyle::DrawSigArea(Double_t xl,Double_t xh,Double_t yl,Double_t
yh){
   ...
   void bes3plotstyle::DrawSide(Double_t xl,Double_t xh,Double_t yl,Double_t
yh){
   TLine *l1 = new TLine(xl,yl,xl,yh);
   TLine *l2 = new TLine(xh,yl,xh,yh);
   TLine *l3 = new TLine(xl,yl,xh,yl);
   ...
   Double_t sigma(RooRealVar sigma1, RooRealVar sigma2, RooRealVar mean1,
RooRealVar mean2 , RooRealVar frac){
   ...
   Double_t mean(RooRealVar sigma1, RooRealVar sigma2, RooRealVar mean1,
RooRealVar mean2 , RooRealVar frac){
   Double_t s1 = sigma1.getVal();
   Double_t s2 = sigma2.getVal();
   Double_t m1 = mean1.getVal();
...
void CalculateResidual( RooPlot *frame, Int_t &bins, Double_t xx[], Double_t
*yy,
    ...
    for(Int_t i = 0; i < _npionts; i++ ){
        ...
        cout<<"data\t"<<_x_data[i]<<" "<<_y_data[i]<<" fit\t"<<x1<<"
"<<y1<<endl;
        curve->GetPoInt_t(idx, x1, y1);
        if(idx ==0){
...
void getSigmaAndMean(RooAbsPdf *sigPdf, TString xname ="deltaE", Double_t &mean,
Double_t &sigma){
    //sigPdf->PrInt_t("v");
